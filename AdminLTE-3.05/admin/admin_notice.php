...
if(isset($_POST['submit_notice'])){
    ...
    mysqli_query($con,"
        INSERT INTO notices
        (title,description,file,course,branch,session,institute_id,class,section)
        VALUES
        ('$title','$description','$file_name','$course_id','$branch_id','$session_id','$institute_id',$class_id','$section_id')
    ");
...
$q = mysqli_query($con,"SELECT * FROM notices $where ORDER BY id ASC");
...
<?php if(!empty($row['file'])){ ?>
    <a href="uploads/annoucement/<?= $row['file'] ?>" target="_blank">
        <img src="uploads/annoucement/<?= $row['file'] ?>"
             width="80"
             height="80"
