...
if ($role != '') {
   ...
   if($class_id != ''){
      $sql .= " AND id IN (
         SELECT user_id FROM usermeta 
         WHERE meta_key='st_class' AND meta_value=?
      )";
      $params[] = $class_id;
      $types .= "s";
   }
   ...
   if($section_id != ''){
      $sql .= " AND id IN (
         SELECT user_id FROM usermeta 
         WHERE meta_key='st_section' AND meta_value=?
      )";
      $params[] = $section_id;
      $types .= "s";
   }
...
if($semester_id!=''){
  ...
  $types .="s";
                       ...
                       while($row=$query_result->fetch_assoc()){ 
                        ...
                        $user_id=$row['id'];
                        if($institute_type=='school'){
                  $user_edit='<a href="teacher.php?class='.$class_id.'&section='.$section_id.'&edit_teacher='.$user_id.'" class ="btn btn-sm btn-success"><i class="fa fa-pencil-alt"></i></a>';

$user_delete='<a href="teacher.php?class='.$class_id.'&section='.$section_id.'&delete_teacher='.$user_id.'" class="btn btn-sm btn-danger mx-2"><i class="fa fa-trash-alt"></i></a>';
...
Assign Subject
             ...
             $user_edit='<a href="teacher.php?course='.$course_id.'&branch='.$branch_id.'&edit_teacher='.$user_id.'" class ="btn btn-sm btn-success"><i class="fa fa-pencil-alt"></i></a>';

$user_delete='<a href="teacher.php?course='.$course_id.'&branch='.$branch_id.'&semester_id='.$semester_id.'&delete_teacher='.$user_id.'" class="btn btn-sm btn-danger mx-2"><i class="fa fa-trash-alt"></i></a>';
