...
$get_course=get_usermeta($user_id,'course_name');
$course = 0;

$course_name = mysqli_query($con,"SELECT id FROM posts WHERE title='$get_course'");
if($course_name && mysqli_num_rows($course_name) > 0){
    $row_course = mysqli_fetch_assoc($course_name);
    $course = $row_course['id'];
}

$get_branch=get_usermeta($user_id,'branch_name');
$branch = 0;

$branch_name = mysqli_query($con,"SELECT id FROM posts WHERE title='$get_branch'");
if($branch_name && mysqli_num_rows($branch_name) > 0){
    $row_branch = mysqli_fetch_assoc($branch_name);
    $branch = $row_branch['id'];
}

$session=get_usermeta($user_id,'session');
$class=get_usermeta($user_id,'class_name');
$section=get_usermeta($user_id,'section_name');
$year=date('Y');
?>
<style>
.important-card {
  ...
  <div class="col-md-4">
   ...
   $select_query=mysqli_query($con,"
        ...
        while($row=mysqli_fetch_assoc($select_query)){
            ...
            $get_publish = date('Y', strtotime($publish));

    if($get_publish != date('Y')){
        continue;
    }
